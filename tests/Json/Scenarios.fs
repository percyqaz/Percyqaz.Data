...
type ``4: Special Deserialisation Scenarios``() =
    ...
    [<Test>]
    member this.Record_Partial() =
        match env.FromString<RecordWithDefault> "{\"X\": 99}" with
        | Ok v ->
            Assert.AreEqual(
                { RecordWithDefault.Default with
                    X = 99L
                },
                v
            )
        ...
        seq {
            for i = 1 to 25 do
                yield
                    async {
                        for j = 1 to 100 do
                            round_trip env (random.NextDouble())
                    }
        }
        |> Async.Parallel
        |> Async.Ignore
