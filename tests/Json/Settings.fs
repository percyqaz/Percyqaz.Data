...
type ``3: Settings tests``() =
    ...
    [<Test>]
    member this.AllowNullArrays() =
        let allow =
            Json(
                { Json.Settings.Default with
                    AllowNullArrays = true
                }
            )
                .WithDefaults()
    ...
    [<Test>]
    member this.FormatExpandArrays() =
        let expand =
            Json(
                { Json.Settings.Default with
                    FormatExpandArrays = true
                }
            )
                .WithDefaults()
    ...
    [<Test>]
    member this.FormatExpandObjects() =
        let expand =
            Json(
                { Json.Settings.Default with
