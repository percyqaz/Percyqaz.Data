...
module Json =
    ...
    module Parsing =
        ...
        let parseStream name stream =
            runParserOnStream jsonParser () name stream Text.Encoding.UTF8

        let parseFile path =
            runParserOnFile jsonParser () path Text.Encoding.UTF8
                ...
                | JSON.Array xs ->
                    let rec f xs =
                        ...
                        | x :: xs ->
                            writeJson x
                            write ","
                            (if expandArray then newline () else write " ")
                            f xs
                ...
                | JSON.Object m ->
                    let rec f xs =
                        ...
                        | (k, x) :: xs ->
                            ...
                            writeJson x
                            write ","
                            (if expandObj then newline () else write " ")
                            f xs
                ...
                fun _ json ->
                    ...
                    | JSON.Number s ->
                        ...
                        if ok then
                            ...
                            failwithf "Parse failure for UInt8: %s" s
                ...
                fun _ json ->
                    ...
                    | JSON.Number s ->
                        ...
                        if ok then
                            ...
                            failwithf "Parse failure for UInt16: %s" s
                    | _ -> failwithf "Expected a JSON number, got: %O" json
                ...
                fun _ json ->
                    ...
                    | JSON.Number s ->
                        ...
                        if ok then
                            ...
                            failwithf "Parse failure for Int16: %s" s
                    | _ -> failwithf "Expected a JSON number, got: %O" json
                ...
                fun _ json ->
                    ...
                    | JSON.Number s ->
                        ...
                        if ok then
                            ...
                            failwithf "Parse failure for UInt32: %s" s
                    | _ -> failwithf "Expected a JSON number, got: %O" json
                ...
                fun _ json ->
                    ...
                    | JSON.Number s ->
                        ...
                        if ok then
                            ...
                            failwithf "Parse failure for Int32: %s" s
                    | _ -> failwithf "Expected a JSON number, got: %O" json
                ...
                fun _ json ->
                    ...
                    | JSON.Number s ->
                        ...
                        if ok then
                            ...
                            failwithf "Parse failure for UInt64: %s" s
                    | _ -> failwithf "Expected a JSON number, got: %O" json
                ...
                fun _ json ->
                    ...
                    | JSON.Number s ->
                        ...
                        if ok then
                            ...
                            failwithf "Parse failure for Int64: %s" s
                    | _ -> failwithf "Expected a JSON number, got: %O" json
        ...
        type Float32() =
            ...
            override this.To(ctx: Context) =
                (fun (f: float32) ->
                    ...
                    if Single.IsNaN f || Single.IsInfinity f then
                        JSON.String s
                    else
                        JSON.Number s
                )
                ...
                fun _ json ->
                    ...
                    | JSON.Number s ->
                        ...
                        if ok then
                            ...
                            failwithf "Parse failure for Float32: %s" s
                    | _ -> failwithf "Expected a JSON number, got: %O" json
                ...
                fun _ json ->
                    ...
                    | JSON.Number s ->
                        ...
                        if ok then
                            ...
                            failwithf "Parse failure for Float64: %s" s
                    | _ -> failwithf "Expected a JSON number, got: %O" json
                ...
                fun _ json ->
                    ...
                    | JSON.Number s ->
                        ...
                        if ok then
                            ...
                            failwithf "Parse failure for Decimal: %s" s
                    | _ -> failwithf "Expected a JSON number, got: %O" json
        ...
        type BigInt() =
            ...
            override this.From(ctx: Context) =
                fun _ json ->
                    ...
                    | JSON.Number s ->
                        let ok, res =
                            Numerics.BigInteger.TryParse(s, NumberStyles.Any, CultureInfo.InvariantCulture)

                        if ok then
                            res
                        else
                            failwithf "Parse failure for BigInteger: %s" s
                    | _ -> failwithf "Expected a JSON number, got: %O" json
        ...
        type String() =
            ...
            override this.To(ctx: Context) =
                if ctx.Settings.AllowNullStrings then
                    ...
                    function
                    | null -> failwith "Null strings not permitted"
                    | s -> JSON.String s
            ...
            override this.From(ctx: Context) =
                ...
                fun l json ->
                    ...
                    | JSON.Array xs ->
                        ...
                        for x in List.map cdc.FromDefault xs do
                            l.Add x
                ...
                if ctx.Settings.AllowNullArrays then
                    fun arr ->
                        if isNull arr then
                            ...
                            Array.map cdc.To arr |> List.ofArray |> JSON.Array
                else
        ...
        type Dictionary<'K, 'V when 'K: comparison>() =
            ...
            override this.To(ctx: Context) =
                if not ctx.Settings.EncodeAllMapsAsArrays && typeof<'K> = typeof<string> then
                    ...
                    Seq.map (|KeyValue|) >> List.ofSeq >> cdc.To
            ...
            override this.Default _ =
                fun _ -> new Collections.Generic.Dictionary<'K, 'V>()
        ...
        type Option<'T>() =
            ...
            override this.From(ctx: Context) =
                ...
                fun existing json ->
                    match json with
                    | JSON.Null -> None
                    | _ -> Some(cdc.From (existing |> Option.defaultWith cdc.Default) json)
        ...
        type ValueOption<'T>() =
            ...
            override this.From(ctx: Context) =
                ...
                fun existing json ->
                    match json with
                    | JSON.Null -> ValueNone
                    | _ -> ValueSome(cdc.From (existing |> ValueOption.defaultWith cdc.Default) json)
        ...
        type Tuple4<'A, 'B, 'C, 'D>() =
            ...
            override this.From(ctx: Context) =
                ...
                fun _ json ->
                    match json with
                    | JSON.Array [ a; b; c; d ] ->
                        (c_a.FromDefault a, c_b.FromDefault b, c_c.FromDefault c, c_d.FromDefault d)
            ...
            override this.From(ctx: Context) =
                ...
                fun _ json ->
                    ...
                    | JSON.Array [ a; b; c; d; e ] ->
                        (c_a.FromDefault a, c_b.FromDefault b, c_c.FromDefault c, c_d.FromDefault d, c_e.FromDefault e)
                    ...
                    | JSON.String s ->
                        let ok, res =
                            DateTime.TryParseExact(
                                s,
                                [| "s"; "r"; "o"; "yyyy'-'MM'-'dd'T'HH':'mm':'ss.FFFFFFFK" |],
                    ...
                    | JSON.String s ->
                        let ok, res =
                            DateTimeOffset.TryParseExact(
                                s,
                                [| "o"; "r"; "yyyy-MM-dd'T'HH:mm:ss.FFFFFFF'Z'" |],
                                CultureInfo.InvariantCulture,
                                DateTimeStyles.AssumeUniversal
                            )
                ...
                fun _ json ->
                    ...
                    | JSON.String s ->
                        ...
                        if ok then
                            ...
                            failwithf "Unable to parse Guid from string: %s, got: %O" s json
                    | _ -> failwithf "Expected a JSON string, got: %O" json
    ...
    module AutoCodecs =
        ...
        let boxed_codec (ctx: Context, ty: Type) =
            ctx
                .GetType()
                .GetMethod(nameof ctx.GetBoxedCodec)
        ...
        let record<'T> (ctx: Context) (requireAll: bool) : CachedCodec<'T> =
            ...
            let fieldRequired =
                ...
                |> Array.map (fun f ->
                    if requireAll then
                        f.GetCustomAttributes(typeof<OptionalAttribute>, false).Length = 0
                    else
            ...
            let defaultFunc =
                if isNull mi then
                    fun () -> codecs |> Array.map (fun x -> x.Default()) |> constructor |> unbox<'T>
                else
                    let d =
                        Default_Delegate<'T>.CreateDelegate(typeof<Default_Delegate<'T>>, mi.GetMethod)
                    ...
                    fun record json ->
                        ...
                        | JSON.Object xs ->
                            ...
                            for i = 0 to fieldNames.Length - 1 do
                                ...
                                elif fieldRequired.[i] then
                                    failwithf "Missing required value: %s, got: %O" fieldNames.[i] json
                ...
                else
                    {
                        To =
                            fun (x: 'T) ->
                                let inner =
                                    reader x
                                    |> Array.map2 (fun cdc v -> cdc.To v) codecs
                                    |> List.ofArray
                                    |> JSON.Array
                        ...
                        From =
                            fun (d: 'T) (json: JSON) ->
                                ...
                                | JSON.Array xs ->
                                    ...
                                    xs
                                    |> Array.ofList
                                    |> Array.map2 (fun (cdc: CachedCodec<obj>) v -> cdc.FromDefault v) codecs
                                    |> constructor
                                    |> unbox<'T>
                                | _ ->
                ...
                From =
                    fun (x: 'T) (json: JSON) ->
                        ...
                        | JSON.Object xs ->
                            ...
                            if i < 0 then
                                failwithf "Unrecognised case name: %s at: %O" (xs.Keys.First()) json
                        ...
                        | JSON.String s ->
                            ...
                            if i < 0 then
                                failwithf "Unrecognised case name: %s at: %O" s json
            ...
            if ctx.Settings.EncodeEnumsAsStrings then
                {
                    To =
                        fun (x: 'T) ->
                            if Enum.IsDefined(ty, x) then
                                JSON.String(x.ToString())
                            else
                                cdc.To x
                    From =
                        fun _ json ->
                            ...
                            | JSON.String s ->
                                ...
                                if ok then
                                    ...
                                    failwithf "Unrecognised enum name: %s at: %O" s json
...
type Json(settings: Settings) as this =
    ...
    member this.WithCodec<'Codec>() =
        ...
        if typeof<Codec<_>>.GetGenericTypeDefinition() = bt.GetGenericTypeDefinition() then
            ...
            failwithf "Type %O must be assignable from Codec<>" ty

    member this.WithAutoCodec<'T>(requireAll: bool) =
        autocodecs.TryAdd(typeof<'T>, requireAll) |> ignore
        this
    ...
    member private this.GenCodec<'T>() : CachedCodec<'T> =
        ...
        if FSharpType.IsRecord ty && ty.Name.Contains("<>f__AnonymousType") then
            ...
            && ty.GetCustomAttributes(typeof<AutoCodecAttribute>, false).Length > 0
        then
            AutoCodecs.record<'T>
                ctx
                (ty.GetCustomAttributes(typeof<AutoCodecAttribute>, false).First() :?> AutoCodecAttribute)
                    .RequireAll
            ...
            Seq.tryPick
                (fun (candidate: Type) ->
                    ...
                    let rec matchup (source: Type) (target: Type) =
                        ...
                        && Array.zip base_args t_args
                           |> Array.forall (fun (source, target) -> matchup source target)

                    if matchup (candidate.BaseType.GetGenericArguments().First()) ty then
                        let gen_type =
                            if type_arguments.Length > 0 then
                                candidate.MakeGenericType(type_arguments)
                            else
                                candidate
        ...
        lock
            ...
            (fun () ->
                ...
                if cache.ContainsKey ty then
                    ...
                    ignore (cache.Remove ty)
                    cache.Add(ty, unbox codec)
                    codec
            )
    ...
    member this.ToString(obj: 'T) =
        obj |> this.ToJson |> Formatting.formatJsonString settings
    ...
    member this.ToFile (path, overwrite) (obj: 'T) =
        if overwrite || not (File.Exists path) then
            ...
            raise <| IOException "This file already exists!"
    ...
    member this.FromFile<'T>(path: string) =
        try
            Ok(Parsing.parseFile path)
        with :? FileNotFoundException as err ->
            Error(exn ("File not found", err))
        |> Result.bind (
            function
            | ParserResult.Success(res, _, _) -> Ok res
            | ParserResult.Failure(_, err, _) -> Error(exn ("Error parsing json", ParseFailure err))
